...
/* Melhorado um pouquinho o codigo, inseri o define e tirei as declaracoes de variaveis no meio nas func,
so a func cadastrar que nao vou refazer, mas sim devia ter sido feito em duas e ter um retorno
tenho ciencia que o codigo nao ficou bom, mas nao deu tempo de melhorar, fiz passar e fiquei fazendo uns testes que queria ver os resultados
*/
...
struct TURMAS{
    char numeroTurma[STRTAM];
    char disciplina[STRTAM];
    char professor[STRTAM];
    int qntAlunos;
    struct ALUNOS *aluno;
...
struct ALUNOS{
        char matricula[STRTAM];
        char nomeAluno[STRTAM];
        double g1, g2, g3;
        double media1;
};
...
void cadastrar(struct TURMAS *turma, int qntTurmas){
    ...
    for(i=0; i < qntTurmas; i++){
        ...
        printf("\n# --- Turma [%i] ---\n", i);
        printf("\n# Codigo da turma: \n");
        lestring(turma[i].numeroTurma, STRTAM);
        //Cadastra o nome da disciplina
        printf("\n# Nome da disciplina: \n");
        lestring(turma[i].disciplina, STRTAM);
        //Cadastra o nome do professor
        printf("\n# Nome do professor: \n" );
        lestring(turma[i].professor, STRTAM);
        ...
        for(j=0; j < turma[i].qntAlunos; j++){
            ...
            lestring(turma[i].aluno[j].matricula, STRTAM);
            printf("\n# Qual o nome do aluno: \n");
            lestring(turma[i].aluno[j].nomeAluno, STRTAM);
